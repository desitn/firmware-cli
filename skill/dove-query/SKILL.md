...
dove.exe monitor --tag Log --timeout 30000
dove.exe monitor --tag Debug --timeout 30000
...
# 按标签选择端口（推荐）
dove.exe at --tag AT -c "ATI"
dove.exe at --tag AT -c "AT+CGMI"
...
- **高风险操作不在此 skill**：flash、build、build-and-flash、设备复位等高风险操作在 `dove-action` skill 中，需要用户确认
